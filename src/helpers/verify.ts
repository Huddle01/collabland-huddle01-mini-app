...
export class SignatureVerifier {
  verify(req: NextApiRequest, res: NextApiResponse) {
    if (!process.env.SKIP_VERIFICATION) {
      const ecdsaSignature = req.headers[ActionEcdsaSignatureHeader];
      const ed25519Signature = req.headers[ActionEd25519SignatureHeader];
      const signatureTimestamp: number = parseInt(
        req.headers[ActionSignatureTimestampHeader]?.toString() ?? "0"
      );
